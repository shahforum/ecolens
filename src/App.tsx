...
type Page = "home" | "scanner" | "about" | "login";
  ...
  return (
    <div className="relative min-h-screen flex flex-col font-sans">
      ...
      <nav className="flex justify-between items-center px-12 py-8 z-50">
        ...
        >
          ...
          <span className="text-xl font-bold tracking-tight text-forest group-hover:text-sage transition-colors">
            EcoLens{" "}
            <span className="font-normal text-sage group-hover:text-forest transition-colors">
              AI
            </span>
          </span>
        </div>
      ...
      <AnimatePresence mode="wait">
        ...
        >
          {currentPage === "home" && (
            <Home onStart={() => setCurrentPage("scanner")} />
          )}
      ...
      <div className="md:hidden fixed bottom-6 left-6 right-6 z-50">
        <div className="bg-forest/90 backdrop-blur-xl p-4 rounded-full flex justify-around items-center text-white/50 text-[10px] font-bold uppercase tracking-widest shadow-2xl">
          <button
            onClick={() => setCurrentPage("home")}
            className={currentPage === "home" ? "text-sage" : ""}
          >
            Home
          </button>
          <button
            onClick={() => setCurrentPage("scanner")}
            className={currentPage === "scanner" ? "text-sage" : ""}
          >
            App
          </button>
          <button
            onClick={() => setCurrentPage("about")}
            className={currentPage === "about" ? "text-sage" : ""}
          >
            Team
          </button>
        </div>
